...
public class OkHttpManager {
    ...
    public void post(List<Param> params, String url, HttpCallBack httpCallBack) {
        if(!NetworkStatus.isCurrentNetworkAvailable) {
            Log.d("info: ", "network unavailable.");
            CommonTools.showShortToast(context, "请检查网络是否可用");
            return;
        }
        Log.d("url----->>", url);
    ...
    public void postMoments(List<Param> params, List<Uri> images, String url, HttpCallBack httpCallBack) {
        ...
        for (int i = 0; i < num; i++) {
            ...
            File file = new File("/sdcard/zhaoyi/" + filename);

            File file_big = new File("/sdcard/zhaoyi/" + "big_" + filename);
