package com.android.view.animation.textsurface;
...
import com.android.view.animation.textsurface.animations.AnimationsSet;
...
import com.android.view.animation.textsurface.interfaces.ISet;
...
public class TextSurface extends FrameLayout {
	...
	private TreeSet<Text> textsTree = new TreeSet<Text>();
	...
	private void configAnimations(ISurfaceAnimation animation) {
		...
		if (animation instanceof ICameraAnimation) {
			...
			LinkedList<ISurfaceAnimation> animations = ((ISet) animation)
					.getAnimations();
	...
	protected void onDraw(Canvas canvas) {
		...
		for (Text text : textsTree)
			text.onDraw(canvas, this);
	}
